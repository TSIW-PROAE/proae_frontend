...
export class InscricaoService {
  ...
  async fetchPagesInformation(editalId: number): Promise<PagesResponse[]> {
    return await this.httpClient.get<PagesResponse[]>(this.url + "/steps/edital/" + editalId, this.headerToken);
  }

  async saveInscricao(answers: Answers) {
    return await this.httpClient.post(this.url + "/inscricoes", answers, this.headerToken);
  }
}
...
export default function Inscricao() {
      ...
      try {
        ...
        pages.forEach((page: PagesResponse) => {
          ...
          page.perguntas.forEach((pergunta: QuestionsResponse) => {
            inputs.push({tipo: pergunta.tipo_Pergunta, titulo: pergunta.pergunta, nome: String(pergunta.id),
              obrigatorio: pergunta.obrigatoriedade, formatacao: pergunta.tipo_formatacao, placeholder: pergunta.placeholder, opcoes: pergunta.opcoes != null ? getOptions(pergunta.opcoes): []});
          })
  ...
  const getOptions = (options: string[]): Option[] => {
    ...
    options.forEach((options => {
      optionsTyped.push({value: options, label: options});
    }))
    return optionsTyped;
  }
